(() => {
  ...
  function historyPreview(item,rawOps){
    const ops=sanitizeOps(rawOps);
    const rows=ops.slice(0,5).map(o=>{const canDelete=o.type==='consumption'||o.type==='writeoff';return `<div class="ic-op"><div><span class="ic-badge t-${esc(o.type)}">${esc(opLabel(o))}</span><small>${new Date(o.created_at).toLocaleString('ru-RU')}${o.reason?` · ${esc(o.reason)}`:''}${o.comment?` · ${esc(o.comment)}`:''}</small></div><div class="ic-op-side"><strong>${opSign(o)}${fmt(safeQuantity(o)??0)} ${esc(o.unit||item.unit||'')}</strong>${canDelete?`<button type="button" class="ic-delete" data-ic-delete="${esc(o.id)}">Удалить</button>`:''}</div></div>`}).join('')||'<div class="ic-empty">Операций пока нет</div>';
  ...
  function enhance(modal,itemId){
    ...
    holder.querySelectorAll('[data-ic-delete]').forEach(b=>b.onclick=e=>{e.stopPropagation();window.KAMBUZ_OPERATIONS?.deleteOperation?.(b.dataset.icDelete)});
  ...
  function renderHistoryRows(box,item,rawOps,filter){
    ...
    box.innerHTML=list.map(o=>{const canDelete=o.type==='consumption'||o.type==='writeoff';return `<div class="ich-row"><div class="ich-top"><span class="ic-badge t-${esc(o.type)}">${esc(opLabel(o))}</span><div class="ich-side"><strong>${opSign(o)}${fmt(safeQuantity(o)??0)} ${esc(o.unit||item.unit||'')}</strong>${canDelete?`<button type="button" class="ic-delete" data-ic-delete="${esc(o.id)}">Удалить</button>`:''}</div></div><div class="ich-meta">${new Date(o.created_at).toLocaleString('ru-RU')} · ${esc(o.user_name||'Пользователь')}${o.reason?` · ${esc(o.reason)}`:''}${o.comment?` · ${esc(o.comment)}`:''}${o.previous_qty!=null&&o.new_qty!=null?` · ${fmt(o.previous_qty)} → ${fmt(o.new_qty)}`:''}</div></div>`}).join('')||'<div class="ich-loading">Нет операций этого типа</div>';
    box.querySelectorAll('[data-ic-delete]').forEach(b=>b.onclick=e=>{e.stopPropagation();window.KAMBUZ_OPERATIONS?.deleteOperation?.(b.dataset.icDelete)});
  }
  ...
  function start(){
    ...
    document.addEventListener('kambuz-operation-deleted',()=>{
      document.getElementById('item-history-v2')?.remove();
      [...document.querySelectorAll('.modal-backdrop')].forEach(x=>{if(x.querySelector('.detail-card'))x.remove()});
    });
